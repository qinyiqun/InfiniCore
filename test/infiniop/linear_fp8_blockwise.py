...
def test(
    ...
    for precision in range (0, len(FP8_SUPPORT_COMBINES)):
        ...
        # for a_1d, b_1d in [(True, False), (False, True), (True, True)]:
            ...
            d = TestTensor(c_shape,None, FP8_SUPPORT_COMBINES[precision][3], device, mode="zeros")
            
            bias = torch.ones(c_shape, device=torch_device_map[device], dtype=c.torch_tensor().dtype)*0.6

            if (a_1d and not b_1d):
                scale_a_ = TestTensor((int (a_shape[0] / 128), int (a_shape[1] / 128)), None, InfiniDtype.F32, device)
                scale_b_ = TestTensor((int (b_shape[1] / 128), int (b_shape[0])), None, InfiniDtype.F32, device)
                pyop_ref_128x128_1x128(a.torch_tensor(), alpha, b.torch_tensor(), beta, scale_a_.torch_tensor(), scale_b_.torch_tensor(), c.torch_tensor(), bias)
            elif (not a_1d and b_1d):
                scale_a_ = TestTensor((int (a_shape[1] / 128), int (a_shape[0])), None, InfiniDtype.F32, device)
                scale_b_ = TestTensor((int (b_shape[0] / 128), int (b_shape[1] / 128)), None, InfiniDtype.F32, device)
                pyop_ref_1x128_128x128(a.torch_tensor(), alpha, b.torch_tensor(), beta, scale_a_.torch_tensor(), scale_b_.torch_tensor(), c.torch_tensor(), bias)
            elif (a_1d and b_1d):
                scale_a_ = TestTensor((int (a_shape[1] / 128), int (a_shape[0])), None, InfiniDtype.F32, device)
                scale_b_ = TestTensor((int (b_shape[1] / 128), int (b_shape[0])), None, InfiniDtype.F32, device)
                pyop_ref_1x128_1x128(a.torch_tensor(), alpha, b.torch_tensor(), beta, scale_a_.torch_tensor(), scale_b_.torch_tensor(), c.torch_tensor(), bias)
            else:
            ...
            assert torch.allclose(d.actual_tensor(), c.torch_tensor().to(d.torch_tensor().dtype), atol=atol, rtol=rtol)
