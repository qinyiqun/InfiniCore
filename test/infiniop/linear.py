...
_TEST_CASES_ = [
    ...
    ((8, 8), (8, 8), (8, 8), 1.0, 0.0),
    ((128, 512), (512, 1024), (128, 1024), 1.0, 0.0),
    ((128, 128), (128, 128), (128, 128), 2.0, 1.0),
    ((256, 1024), (1024, 2048), (256, 2048), 1.0, 1.0),
    ((256, 2048), (2048, 1024), (256, 1024), 1.5, 2.5),
]
...
def quantWeights(w: torch.Tensor, axis):
    """
...
def test(
    ...
    print(
        f"Testing Linear on {InfiniDeviceNames[device]} with x_shape:{x_shape}, w_shape:{w_shape}, alpha:{alpha}, beta:{beta} dtype:{InfiniDtypeNames[dtype]}"
    )
    M, K = x_shape
    N = w_shape[1]
    ...
    x_p, x_s, x_z = quantWeights(x.torch_tensor(), 1)
    ...
    w_packed, w_scale, w_zero = quantWeights(w.torch_tensor(), 0)
    ...
    check_error(
        LIBINFINIOP.infiniopCreateLinearDescriptor(
            handle, ctypes.byref(descriptor), d.descriptor, y.descriptor, bias.descriptor, x_packed.descriptor, x_scale.descriptor, x_zero.descriptor, weights.descriptor, weights_scale.descriptor, weights_zero.descriptor, alpha, beta
        )
    )
