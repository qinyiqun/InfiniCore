...
#include "nvidia/rms_norm_backward_nvidia.cuh"
#endif
...
__C infiniStatus_t infiniopRMSNormBackward(
...
#define CALCULATE(CASE, NAMESPACE)                                                                            \
    case CASE:                                                                                                \
        return reinterpret_cast<const op::rms_norm_backward::NAMESPACE::Descriptor *>(desc)->calculate(       \
            workspace,                                                                                        \
            workspace_size,                                                                                   \
            grad_x,                                                                                           \
            grad_w,                                                                                           \
...
__C infiniStatus_t
infiniopDestroyRMSNormBackwardDescriptor(infiniopRMSNormBackwardDescriptor_t desc) {

#define DELETE(CASE, NAMESPACE)                                                                     \
    case CASE:                                                                                      \
        delete reinterpret_cast<const op::rms_norm_backward::NAMESPACE::Descriptor *>(desc);        \
        return INFINI_STATUS_SUCCESS
