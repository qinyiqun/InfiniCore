...
public:
    ...
    static utils::Result<LinearInfo> createLinearInfo(
        ...
        CHECK_OR_RETURN(
            d_desc != nullptr && c_desc != nullptr && bias_desc != nullptr && x_desc != nullptr && x_scale_desc != nullptr && x_zero_desc != nullptr && weights_desc != nullptr && weights_scale_desc != nullptr && weights_zero_desc != nullptr,
            INFINI_STATUS_NULL_POINTER);

        const infiniDtype_t dtype = d_desc->dtype();
        const infiniDtype_t packed_type = x_desc->dtype();
        CHECK_OR_RETURN(dtype == c_desc->dtype() && dtype == bias_desc->dtype() && dtype == x_scale_desc->dtype() && dtype == x_zero_desc->dtype() && dtype == weights_scale_desc->dtype() && dtype == weights_zero_desc->dtype(),
