...
#define CREATE(CASE, NAMESPACE)                                                                     \
    case CASE:                                                                                      \
        return op::batch_norm_backward::NAMESPACE::Descriptor::create(                              \
            handle,                                                                                 \
            reinterpret_cast<op::batch_norm_backward::NAMESPACE::Descriptor **>(desc_ptr),          \
            grad_input_desc,                                                                        \
            grad_weight_desc,                                                                       \
            grad_bias_desc,                                                                         \
            input_desc,                                                                             \
...
__C infiniStatus_t infiniopGetBatchNormBackwardWorkspaceSize(infiniopBatchNormBackwardDescriptor_t desc, size_t *size) {
#define GET(CASE, NAMESPACE)                                                                                  \
    case CASE:                                                                                                \
        *size = reinterpret_cast<op::batch_norm_backward::NAMESPACE::Descriptor *>(desc)->workspaceSize();    \
...
#define CALCULATE(CASE, NAMESPACE)                                                                            \
    case CASE:                                                                                                \
        return reinterpret_cast<const op::batch_norm_backward::NAMESPACE::Descriptor *>(desc)->calculate(     \
            workspace,                                                                                        \
            workspace_size,                                                                                   \
            grad_input,                                                                                       \
            grad_weight,                                                                                      \
...
__C infiniStatus_t
infiniopDestroyBatchNormBackwardDescriptor(infiniopBatchNormBackwardDescriptor_t desc) {

#define DELETE(CASE, NAMESPACE)                                                                     \
    case CASE:                                                                                      \
        delete reinterpret_cast<const op::batch_norm_backward::NAMESPACE::Descriptor *>(desc);      \
        return INFINI_STATUS_SUCCESS
