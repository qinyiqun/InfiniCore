...
#define CREATE_ELEMENTWISE_CPU_DESCRIPTOR(HANDLE, DTYPE, OUT_DESC,             \
                                          INPUT_DESC_VEC)                      \
                                                                               \
  auto info_result =                                                           \
      op::elementwise::ElementwiseInfo::create(OUT_DESC, INPUT_DESC_VEC);      \
    ...
    auto get_input_idx = [&](size_t input_id) {
      return info.getInputContiguous()[input_id]
                 ...
                 : (info.getInputBroadcasted()[input_id]
                        ? op::common_cpu::indexToReducedOffset(
                              i, info.getNdim(), info.getOutputStrides(),
...
#pragma omp parallel for
  for (ptrdiff_t i = 0; i < output_size; ++i) {
    ...
    if constexpr (std::is_same_v<Tdata, fp16_t> ||
      ...
      out[out_idx] =
          Op{}(ins[Is][get_input_idx(Is)]..., std::forward<Args>(args)...);
    }
  }
}
